...
export async function extractCvWithAnthropic(
  ...
  return extractWithSingleReinspection({
    initial: () => invokeExtraction(client, document, EXTRACTION_ANALYSIS_PROMPT),
    reinspect: (prompt) => invokeExtraction(client, document, prompt),
  });
}
